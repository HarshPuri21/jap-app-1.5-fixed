...
class SettingsScreen extends StatelessWidget {
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    // --- Study -------------------------------------------
                    const ThemedSectionLabel('Study'),
                    ThemedSurface(
                      level: SurfaceLevel.standard,
                      radius: t.radii.md,
                      padding: EdgeInsets.all(t.spacing.sm),
                      child: _goalSliderRow(context),
                    ),
  ...
  /// Today's Daily Review goal, used by the home screen's Daily Progress
  /// card and streak calculation. An integer stepper dressed as a slider
  /// row, matching the volume/dimness sliders above it.
  Widget _goalSliderRow(BuildContext context) {
    final t = context.tokens;
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(
                  'Daily review goal — items per day',
                  style: t.text.secondary,
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
          ...
          Slider(
            value: stats.dailyGoal.toDouble(),
            min: 5,
            max: 50,
            divisions: 9,
            onChanged: (v) =>
                context.read<StatsService>().setDailyGoal(v.round()),
          ),
        ],
      ),
    );

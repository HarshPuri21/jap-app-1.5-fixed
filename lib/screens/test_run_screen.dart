...
/// One completed question, kept around so the results screen can show a
/// full review -- prompt, what you picked, and the correct answer -- only
/// once the whole test is over.
...
class _TestRunScreenState extends State<TestRunScreen> {
  ...
  final List<TestReviewEntry> _review = [];
  ...
  /// Registers the tap only -- correctness is recorded silently for the
  /// final score, but nothing on screen shows right or wrong until the
  /// whole test is complete (see TestReviewEntry / TestResultsScreen).
  void _choose(String option) {
    ...
    setState(() {
      ...
      if (correct) _score += 1;
      _review.add(TestReviewEntry(
        prompt: _current.prompt,
        isSingleChar: _current.isSingleChar,
        chosen: option,
        answer: _current.answer,
      ));
    });
    // A neutral click either way -- the sound itself must not give away
    // whether the pick was right.
    context.read<AudioService>().playLessonClick();
    // Lifetime accuracy (Settings > Your progress) is a background counter
    // the player never sees mid-test, so recording it here doesn't leak
    // anything about this question's answer.
  ...
  /// Only ever "picked" (neutral) or "idle" during the run -- never reveals
  /// which option was actually correct until the results screen.
  OptionState _stateFor(String option) {
    if (_selected == null) return OptionState.idle;
    if (option == _selected) return OptionState.selectedNeutral;
  ...
  Widget _buildAppBar(BuildContext context) {
    return ThemedAppBar(
      ...
      // A question counter, not a running score -- showing the score live
      // would give away whether each answer was right as you went.
